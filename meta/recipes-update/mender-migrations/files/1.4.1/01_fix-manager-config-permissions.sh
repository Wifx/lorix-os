...
MAN_SRC_CONFIG_DYN="manager/conf.d/01-dynamic" 

# Check if file exists
if [[ -d "$MAN_SRC_CONFIG_DYN" ]]; then
    log $PREFIX "Migrating manager config permissions..."
    chmod 755 $MAN_SRC_CONFIG_DYN
else
    log $PREFIX "No manager config to migrate"
fi
