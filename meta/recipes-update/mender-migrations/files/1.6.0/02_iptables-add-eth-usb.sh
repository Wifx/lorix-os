...
FW_CONFIG="iptables/iptables.rules"

if [[ ! -f "$FW_CONFIG" ]]; then
    log $PREFIX "No IPv4 config to migrate (/etc/$FW_CONFIG)"
    exit 0
fi

log $PREFIX "Migrating IPv4 firewall configuration at /etc/$FW_CONFIG..."
...
USB="[0:0] -A INPUT -i usb0 -j ACCEPT -m comment --comment \"Accept everything on usb0\""
sed -i "/$LO/a $USB" $FW_CONFIG
