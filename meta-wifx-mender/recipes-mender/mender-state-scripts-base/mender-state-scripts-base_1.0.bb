...
SRC_URI = " \
    ...
    file://setup-env.sh;subdir=${PN}-${PV} \
    file://setup-utils.sh;subdir=${PN}-${PV} \
    file://check-version.sh;subdir=${PN}-${PV} \
    file://check-space.sh;subdir=${PN}-${PV} \
    file://setup-migration.sh;subdir=${PN}-${PV} \
    file://migrate.sh;subdir=${PN}-${PV} \
    file://migrate-cleanup.sh;subdir=${PN}-${PV} \
    file://migrate-reset-immutables.sh;subdir=${PN}-${PV} \
    file://setup-migration-cleanup.sh;subdir=${PN}-${PV} \
    file://logs-save.sh;subdir=${PN}-${PV} \
    file://logs-restore.sh;subdir=${PN}-${PV} \
    file://config-migration-disable.sh;subdir=${PN}-${PV} \
    file://config-migration-enable.sh;subdir=${PN}-${PV} \
    file://version-compare.run;subdir=${PN}-${PV} \
"
