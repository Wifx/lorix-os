...
do_image_mender[depends] += "makeself-native:do_populate_sysroot"
    ...
    for module in $(ls ${IMAGE_ROOTFS}/${datadir}/mender/modules/v3); do
        if [ -x "${IMAGE_ROOTFS}/${datadir}/mender/modules/v3/${module}" ]; then
            modules_provides="${modules_provides} \
                ...
                rootfs-image.update-module.${module}.checksum:${img_checksum} \
...
IMAGE_CMD:mender () {
    ...
    if [ -f "${IMGDEPLOYDIR}/${ARTIFACTIMG_NAME}.bootstrap-artifact" ]; then
        mkdir -p "${WORKDIR}/bootstrap-artifact/archive"
        cp "${IMGDEPLOYDIR}/${ARTIFACTIMG_NAME}.bootstrap-artifact" "${WORKDIR}/bootstrap-artifact/archive/bootstrap.mender"
        makeself.sh --noprogress \
            --target /tmp/mender/bootstrap-artifact \
            "${WORKDIR}/bootstrap-artifact/archive" \
...
IMAGE_TYPEDEP:mender:append = " ${ARTIFACTIMG_FSTYPE} bootstrap-artifact"
